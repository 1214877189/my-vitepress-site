...
![image-20250619102651808](./assets/image-20250619102651808.png)
...
![image-20250619102817297](./assets/image-20250619102817297.png)

镜像选择CentOS核Ubuntu都可以，文档里使用了Ubuntu。选择后点击下一步。

![image-20250619103252815](./assets/image-20250619103252815.png)

![image-20250619103307728](./assets/image-20250619103307728.png)

拉满带宽上限，新建安全组，把常用的端口都开启

![image-20250619103457368](./assets/image-20250619103457368.png)

命名实例，设置密码，进行下一步

![image-20250619103548187](./assets/image-20250619103548187.png)

开通

![image-20250619103615844](./assets/image-20250619103615844.png)

#### 2.1.2 登陆服务器

创建好了，通过这个公网IP，端口使用22，账号ubuntu，密码使用你设置的密码。使用你的远程连接工具连接即可

![image-20250619103844777](./assets/image-20250619103844777.png)

#### 2.1.3 添加FTP安全组

腾讯云对FTP上传文件有一些限制，我们需要进行更改才能上传文件

![image-20250619112542899](./assets/image-20250619112542899.png)



![image-20250619112542899](./assets/image-20250619112542899.png)

![image-20250619112558136](./assets/image-20250619112558136.png)

![image-20250619112615974](./assets/image-20250619112615974.png)

![image-20250619112659812](./assets/image-20250619112659812.png)
...
```bash
sudo passwd root # 给root用户设置密码，一般就设置为root
su root # 切换到root
```

客户端连接使用root用户

![image-20250619113345673](./assets/image-20250619113345673.png)
...
# 安装 Docker CE, CLI, 容器化工具和 Docker Compose 插件
sudo apt-get install docker-ce docker-ce-cli containerd.io docker-compose-plugin

```

如果安装docker 的时候出现这个界面，直接回车就可以！
...
```bash
sudo systemctl start docker  # 启动dock
sudo systemctl status docker # 查看docket状态
```

​	如图所示即为启动成功

![](./assets/image-20250619104758997.png)
...
![image-20250619105104648](./assets/image-20250619105104648.png)
...
![image-20250619105258790](./assets/image-20250619105258790.png)
...
利用FTP工具将下面文件上传到在此目录下

![image-20250619110235982](./assets/image-20250619110235982.png)
...
![image-20250619115614270](./assets/image-20250619115614270.png)
...
![image-20250619120100518](./assets/image-20250619120100518.png)

现在配置全部搞定，在docker目录下开始启动

![image-20250619134637133](./assets/image-20250619134637133.png)
...
![image-20250619134716042](./assets/image-20250619134716042.png)

安装完毕

![image-20250619135921801](./assets/image-20250619135921801.png)
...
![image-20250619135912891](./assets/image-20250619135912891.png)

- 先注册一个账号

![image-20250619140313102](./assets/image-20250619140313102.png)

![image-20250619140339250](./assets/image-20250619140339250.png)

- 然后登陆

![image-20250619140358858](./assets/image-20250619140358858.png)

- 设置一下中文

![image-20250619140428425](./assets/image-20250619140428425.png)



#### 2.4.2. 生成ragFlow的API KEY
...
![image-20250619140855488](./assets/image-20250619140855488.png)

- 点击模型供应商，下滑寻找Tongyi-Qianwen

![image-20250619140605716](./assets/image-20250619140605716.png)

点击添加模型，然后把自己的API-KEY粘贴进去

- 设置默认模型

![image-20250619141933424](./assets/image-20250619141933424.png)

设置自己的模型，千问系列的优点是模型十分全面，大语言模型、多模态模型、重排模型、向量化模型都可以使用，只需要配置其API就可以完成大多数任务需求。

![image-20260607222815782](.\assets\image-20260607222815782.png)

#### 2.4.4 创建知识库，并上传文件

设置完成后，就需要开始配置自己的知识库了。这里准备好了一些测试使用的知识库文件，大家可以直接使用，也可以根据自己的兴趣去寻找收集其他知识文档。

![image-20250619144708406](./assets/image-20250619144708406.png)

- 创建知识库

![image-20250619144741192](./assets/image-20250619144741192.png)

- 划到最下面点击保存即创建成功

![image-20250619144812672](./assets/image-20250619144812672.png)

- 上传文件到知识库

新增文件，把准备好的文件进行上传，**上传后务必点击解析进行文档解析！**这里的pdf是调用多模态模型进行分析，可能会耗时比较长，请耐心等待

![image-20250619144958440](./assets/image-20250619144958440.png)

解析成功，如果解析失败了就一个一个再重新解析。

![image-20250619145801880](./assets/image-20250619145801880.png)

#### 2.4.5. 创建知识助手和会话

**注意，这里的助理描述一定要详细填写，否则在后续调用API的时候会出现无法灵活处理问题的情况**

![image-20250619150034877](./assets/image-20250619150034877.png)

​	这里创建完成后新建一个对话，做出一个简单的相关问题提问，回答中会给出每一条信息的来源标识，并且会给出源文件。可见，其并不是靠着自己的强大模型能力完成的，而是根据知识库的信息检索输出的。

![image-20250619150230019](./assets/image-20250619150230019.png)

![image-20250619150315666](./assets/image-20250619150315666.png)

之后将这几种数据文件全部新建知识库，并且创建知识助手。可以让一个助手掌握一个或多个知识库，根据具体情况灵活调整即可。

依次创建成功

![image-20250619152749961](./assets/image-20250619152749961.png)

创建出助手进行测试

![image-20250619153004516](./assets/image-20250619153004516.png)
...
![image-20250620105319201](./assets/image-20250620105319201.png)
...
![image-20250620105521620](./assets/image-20250620105521620.png)

- 删除关键字

![image-20250620105701633](./assets/image-20250620105701633.png)
